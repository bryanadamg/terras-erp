...
export default function UOMView({ uoms, onCreateUOM, onDeleteUOM, onRefresh }: any) {
  ...
  const handleCreate = (e: React.FormEvent) => {
      e.preventDefault();
      if (newUOMName.trim()) {
          onCreateUOM(newUOMName.trim());
          setNewUOMName('');
      }
  };

  const filtered = (uoms || []).filter((u: any) =>
                  ...
                  <div style={xpBevel}>
                      ...
                      <form onSubmit={handleCreate}>
                          <div style={{ ...xpToolbar, borderBottom: '1px solid #b0a898' }}>
                              <span style={{ fontFamily: 'Tahoma,Arial,sans-serif', fontSize: '11px', color: '#000', whiteSpace: 'nowrap' }}>New UOM:</span>
                              <input
                                  style={{ ...xpInput, flex: 1, minWidth: 120 }}
                                  placeholder="e.g. Dozen, Box, Litre..."
                                  value={newUOMName}
                      ...
                      <div style={{ background: '#ffffff', maxHeight: 'calc(100vh - 320px)', overflowY: 'auto' }}>
                          {filtered.map((uom: any, i: number) => (
                              <div key={uom.id} style={{
                                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                  padding: '4px 8px',
                                  background: i % 2 === 0 ? '#ffffff' : '#f5f3ee',
                                  borderBottom: '1px solid #c0bdb5',
                              }}>
                                  <span style={{ fontFamily: 'Tahoma,Arial,sans-serif', fontSize: '11px', color: '#000', fontVariant: 'all-small-caps' }}>{uom.name}</span>
                                  <button
                                      style={{
                                          ...xpBtn(),
                                          border: hoveredId === uom.id ? '1px solid #808080' : '1px solid transparent',
                                          background: hoveredId === uom.id ? 'linear-gradient(to bottom,#ffffff,#d4d0c8)' : 'transparent',
                                          padding: '1px 6px',
                                      }}
                                      onMouseEnter={() => setHoveredId(uom.id)}
                      ...
                      <div style={{
                          ...
                          <span><b>{(uoms || []).length}</b> Total</span>
                      </div>
                  </div>
              </div>
          </div>
              ...
              <div className="card h-100 shadow-sm border-0">
                  ...
                  <div className="card-body">
                      ...
                      <div className="input-group mb-3">
                          ...
                          <input className="form-control" placeholder="Search units..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                      </div>
                      <div className="list-group list-group-flush border rounded">
                          {filtered.map((uom: any) => (
                              <div key={uom.id} className="list-group-item d-flex justify-content-between align-items-center">
                                  ...
                                  <button className="btn btn-sm text-danger" onClick={() => onDeleteUOM(uom.id)}>
