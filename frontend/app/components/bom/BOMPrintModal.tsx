...
export default function BOMPrintModal({ bom, companyProfile, getAttributeValueName, onClose }: Props) {
    ...
    return (
        <>
            <PrintModalShell modeless title={`Print BOM — ${bom.code}`} onClose={onClose} width="calc(var(--app-vw) * 90 / 100)" maxWidth={980} height="calc(var(--app-vh) * 88 / 100)">
                    {/* Body */}
                    ...
                    <PrintModalFooter
                        ...
                        onClose={onClose}
                        onPrint={() => { window.addEventListener('afterprint', onClose, { once: true }); window.print(); }}
                    />
