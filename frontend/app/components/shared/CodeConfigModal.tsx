...
export function getDefaultSegments(type: string): Segment[] {
  const defaultPrefixes: Record<string, string> = {
    BOM: 'BOM', WO: 'WO', MO: 'MO', PO: 'PO', SO: 'SO', SAMPLE: 'SMP', ITEM: 'ITM', DYE: 'DR',
  };
...
function getTypeName(type: string): string {
  const names: Record<string, string> = {
    BOM: 'BOM', WO: 'Work Order', MO: 'Manufacturing Order', PO: 'Purchase Order',
...
interface CodeConfigModalProps {
    isOpen: boolean;
    onClose: () => void;
    type: 'BOM' | 'WO' | 'MO' | 'PO' | 'SO' | 'SAMPLE' | 'ITEM' | 'DYE';
