...
test('Full ERP Lifecycle: Buy, Make, Sell', async ({ page }) => {
  ...
  for (let i = 0; i < attrCount; ++i) {
      await attrSelects.nth(i).selectOption({ index: 1 }); // Select first non-placeholder option
  }

  // Automate Structure
  await page.getByTestId('automate-levels-btn').click();
  // Wait for Automator modal
  await expect(page.getByTestId('bom-automator-modal')).toBeVisible();
  await page.getByTestId('generate-structure-btn').click();
  // Automator modal should close
  await expect(page.getByTestId('bom-automator-modal')).toBeHidden();
  ...
  // We need to buy RM-A, RM-B AND the auto-generated WIP item to ensure stock for production
  ...
  // Add Line: WIP Item (Generated by automation)
  await page.click('text=Select Item...');
  ...
  const attrCount2 = await attributeSelects.count();
  for (let i = 0; i < attrCount2; ++i) {
